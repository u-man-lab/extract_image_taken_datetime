...
両方のスクリプトは、`configs/`ディレクトリ内のYAML設定ファイルから設定を読み込み、入力CSVファイルが`data/`ディレクトリ内に存在することを前提としています。結果は`results/`ディレクトリに書き込まれます。
...
[`extract_image_taken_datetime.py`](./extract_image_taken_datetime.py) は、CSVファイルからファイルパスの一覧を読み込み、ExifToolを使用して最も関連性の高い「撮影日時」を決定します。
優先順位順に並べられた日時関連のタグ一覧を使用します。タグの優先順位は、設定用のYAMLファイルに既に定義されています。ファイルに設定されているメタデータタグの一覧を確認したい場合は、まず[`read_exiftool_values_of_files.py`](#2-read_exiftool_values_of_filespy)を実行してください。
...
ExifToolがインストールされ、PATHが通すなどして"exiftool"コマンドにより利用可能である必要があります。  
...
2. **Specific tags mode** — YAML設定ファイルに`PROCESS` > `TARGET_EXIFTOOL_TAGS`を設定すると、結果のCSVファイルに指定したExifToolタグと実際の値が表示されます。

---

### 2.2. 使い方

実行する前に、以下の準備が完了していることを確認してください：
...
（設定の詳細は[1章](#1-extract_image_taken_datetimepy)を参照してください。）
